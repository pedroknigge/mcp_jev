...
`MCP_JEV_HOME` (default `~/mcp_jev`) is the git checkout. `MCP_JEV_CONFIG` (default `~/.mcp_jev`) is the key + wrapper. Pedro Desktop example: `MCP_JEV_HOME=/Users/pedroknigge/Desktop/mcp_jev ./scripts/install.sh`.
