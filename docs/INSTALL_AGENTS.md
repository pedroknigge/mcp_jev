...
`mcp_jev doctor` → `mcp_jev smoke` (or `scripts/verify-mcp.sh`; no TypeSafe) → host `ping` → `list_packs` → `describe_pack` / `run_pack`, or `run_questions` if no pack fits. `npm test` never needs a live key. Custom path: [CUSTOM_JUDGMENTS.md](CUSTOM_JUDGMENTS.md).
